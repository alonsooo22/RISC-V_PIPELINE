...
### 数据前递/冒险处理的实际实现
- **阻塞式解决数据相关**: RTL中并未实现独立的旁路多路复用器，数据相关通过`vsrc/src/core.sv`里的`stall`判定冻结流水线：`stall = (mem_exception || ((id_rs1 != 0) && (id_rs1 == mem_rd)) || ((id_rs2 != 0) && (id_rs2 == mem_rd)))`。当ID阶段源寄存器命中处于MEM阶段的目的寄存器时，IF/ID/EX/MEM/WB整体被`block`信号保持。
- **依赖写回后的寄存器读取**: 实际可用的数据前递路径是WB阶段同步写回到`regfile` (`vsrc/src/WB_stage.sv`)，而EX阶段对寄存器是组合读 (`vsrc/src/EX_stage.sv`中`regfile[rs1]`/`regfile[rs2]`)。因此后续指令只能在前一条指令写回后读取最新值，未实现典型的ALU/MEM到EX的旁路网络，数据相关通过流水线暂停来避免RAW冒险。
