...
#### 前递网络在本代码中的实现方式
- **统一寄存器堆旁路**: 本实现中寄存器地址在 ID 产生，而寄存器值在 EX（`EX_stage.sv`）通过组合逻辑直接读取 `regfile[rs1]`、`regfile[rs2]`。`core.sv` 将同一份 `regfile[31:0]` 同时连接到 EX/WB，因此消费者指令进入 EX 时可读到此前已写回到寄存器堆的最新值。
- **写回源多路选择**: `WB_stage.sv` 根据指令类型选择写回数据源（ALU结果/访存结果/CSR读出值）并写入 `regfile[rd]`，构成“生产者→寄存器堆→消费者(EX)”的数据前递路径。
- **Load-Use 冒险停顿**: `core.sv` 中检测 `id_rs1/id_rs2` 与 `mem_rd` 冲突并产生 `stall/block`。当依赖数据尚在访存返回路径上时，流水线会保持停顿直到相关数据可用（停顿时长由实际访存返回时机决定）。
- **x0 特判**: 相关性检测时排除 `rs==0`，避免对零寄存器产生无意义前递/停顿。
- **结论**: 该实现属于“寄存器堆可见性前递 + Load-Use停顿”的组合方案：依赖值先写回寄存器堆，再由后续进入 EX 的指令读取；若数据尚未返回（如 load-use），则通过停顿等待可用。与常见“EX/MEM 直接旁路到 ALU 输入”的前递网络相比，这里更依赖寄存器堆写回可见性与停顿控制来保证正确性。
